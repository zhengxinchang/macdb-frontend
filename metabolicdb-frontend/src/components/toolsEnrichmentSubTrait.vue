<template>
  <div>
    <v-sheet
      ...
      class="mt-16"
    >
      <tools-enrichment-common-echarts chartid="enrichment_trait_id" :options="chartOption"> </tools-enrichment-common-echarts>
    </v-sheet>
    ...
    <v-sheet v-show="isok == true" >
      ...
      <vxe-table
        ...
        max-height="500"
      >
        <vxe-column sortable field="trait_ontology" width="450" title="Trait">
        ...
        <vxe-column type="expand" field="hits_data"  title="Metabolites Hits" >
          <template #content="{row}">
            <v-sheet min-width="100" class="text-left pa-3">
              <v-chip class="ma-1" @click="$commonfunc.openAtNewPageMetaboliteDetail(item)"  label small v-for="item in row.hits_data " :key="item"  outlined :color="$store.state.mainColor">
                {{item}}
              </v-chip>
<!--            {{row.hits_data}}-->
            </v-sheet>
        ...
        <vxe-column sortable field="trait_type"  title="Trait Type" ></vxe-column>
        <vxe-column sortable field="efo_ontology"  title="EFO Ontology" ></vxe-column>
        <vxe-column sortable field="EFO_id"  title="EFO Ontology ID" ></vxe-column>
      ...
      if(this.isok ==  true){
        ...
        Axios.get(baseURL+"/api/tool_enrichment_get_data",{
          ...
          this.chartData.data = this.datTop.map(item=>{
            ...
            return [item.trait_ontology,item.adjp,item.hits]
          })
          this.chartData.maxDotSize = _.maxBy(this.datTop ,"hits")['hits']
          this.chartData.maxPval = _.maxBy(this.datTop ,"adjp")['adjp']
          // this.pagination.totalPage = this.dat && this.dat.length || 10
